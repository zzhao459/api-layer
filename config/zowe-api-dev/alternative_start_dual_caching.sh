...
echo "starting CS1"
_BPX_JOBNAME=${ZOWE_PREFIX}C1 java -javaagent:${ROOT_DIR}/components/api-mediation/elastic-apm-agent.jar -Xms16m -Xmx512m -Xquickstart \
  -Dibm.serversocket.recover=true \
  -Dfile.encoding=UTF-8 \
  -Djava.io.tmpdir=/tmp \
  -Dspring.profiles.include=$LOG_LEVEL \
  ...
  -Delastic.apm.service_name=CACHING-SERVICE-1 \
...
_BPX_JOBNAME=${ZOWE_PREFIX}C2 java -javaagent:${ROOT_DIR}/components/api-mediation/elastic-apm-agent.jar -Xms16m -Xmx512m -Xquickstart \
  ...
  -Djava.io.tmpdir=/tmp \
  -Dspring.profiles.include=$LOG_LEVEL \
  -Dapiml.service.port=$((CACHING_SERVICE_PORT+1)) \
  -Dapiml.service.hostname=${ZOWE_EXPLORER_HOST} \
  -Dapiml.service.discoveryServiceUrls=https://${ZOWE_EXPLORER_HOST}:${DISCOVERY_PORT}/eureka/ \
  -Dapiml.service.ipAddress=${ZOWE_IP_ADDRESS} \
  -Dapiml.service.customMetadata.apiml.gatewayPort=${GATEWAY_PORT} \
  ...
  -Dcaching.storage.mode=vsam \
  -Dcaching.storage.vsam.name="//'JANDA06.CACHE5'" \
  -Delastic.apm.service_name=CACHING-SERVICE-2 \
  -Delastic.apm.server_urls=http://mundev001682.bpc.broadcom.net:8200 \
  -Delastic.apm.application_packages=org.zowe \
  -Delastic.apm.trace_methods=org.zowe.* \
  -jar ${ROOT_DIR}"/components/api-mediation/caching-service.jar" &
